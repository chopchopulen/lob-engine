...
| Real end-to-end throughput: ~19.5M msg/s single-pass-equivalent, ~14.68s avg full run (5 study tickers, 2019-12-30, Apple M3 Pro), after the locate-scan early-exit | `bench/BASELINE.md` "Locate-scan early-exit — implemented" | An earlier draft of this figure ("~18.8k msg/s") was a mismatched-denominator artifact (filtered-match-count numerator over full-file-scan-time denominator) — diagnosed, confirmed against the code, corrected. `main.cpp`'s timer fixed to cover the whole run. Locate resolution (`parse_stock_directory`) previously scanned the whole 8.25GB file for `'R'` messages that all sit in the first 0.0044% of it — now early-exits on a structural marker (first System Event other than 'O'), with a hard-error safeguard (not a silent skip) if a late `'R'` ever violates that guarantee, covered by 2 new tests (16 total, was 14). Measured ~1.88x end-to-end speedup (27.63s→14.68s avg), less than the naive 2x due to page-cache effects — measured, not assumed. Hot path/microbenchmarks confirmed unaffected. Distinct from the retired 226M/7.8M claim |
